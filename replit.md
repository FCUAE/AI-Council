...
- **Sign-in/Sign-up**: Uses `clerk.openSignIn()` / `clerk.openSignUp()` with try/catch fallback to `clerk.redirectToSignIn()` / `clerk.redirectToSignUp()`. Both sidebar and header auth buttons pass `afterSignInUrl`/`afterSignUpUrl` for consistent post-auth redirect. No Clerk wrapper components (`SignInButton`/`SignUpButton`) are used.
...
### Security
- **CSP**: Full Content Security Policy via Helmet with allowlists for Clerk, Stripe, RefGrow, Google Fonts, and Replit WebSocket domains. `object-src: 'none'`, `base-uri: 'self'`, `frame-ancestors: 'self'`. **IMPORTANT**: Dev and prod Clerk keys resolve to different Frontend API hostnames — dev uses `stunning-beetle-45.clerk.accounts.dev`, prod uses `clerk.askaicouncil.com`. Both must be in every Clerk-related CSP directive (script-src, style-src, img-src, connect-src, frame-src, form-action). If you add a new Clerk instance or change the custom domain, decode the publishable key (base64 after `pk_live_` / `pk_test_` prefix) to find the hostname and add it to the CSP.
- **Referrer-Policy**: `strict-origin-when-cross-origin` via Helmet config
- **Permissions-Policy**: Disables camera, microphone, geolocation, interest-cohort via custom middleware after Helmet
- **SRI**: Refgrow affiliate script (`scripts.refgrowcdn.com/page.js`) loaded with `integrity` (sha384) and `crossOrigin="anonymous"`
- **CSRF protection**: Origin header validation middleware on all state-changing API requests (`POST/PUT/PATCH/DELETE` to `/api/`). Missing Origin headers denied when allowed origins are configured. Stripe webhook excluded.
- **Rate limiting** (`express-rate-limit` + per-user):
  ...
  - Retry: 3/min per-user; Extract-text: 10/min per-user; Stripe recovery/sync: 3/min per-user
  - Stripe checkout: 10 requests per minute per IP
  - Per-user in-memory rate limiter in `server/routes.ts` supplements IP-based limits, keyed as `${userId}:${route}` for route-scoped isolation
...
- **Attachment authorization**: Centralized `server/security/attachmentAuth.ts` validates all attachment URLs at ingestion time (before DB write). URLs are normalized and canonicalized before ownership/ACL checks. `/uploads/*` requires `file_uploads` ownership record; `/objects/*` requires `canAccessObjectEntity` ACL; external URLs require `isUrlSafeForFetch` allowlist (shared utility in `server/utils/urlSafety.ts`). Retry fails closed on malformed/unauthorized stored attachments (before CAS mutation). Defense-in-depth checks in `imageUrlToBase64` with userId and isAdminUser parameters threaded through the full processing chain.
- **File upload security**: Magic-byte validation, extension blocklist, filename sanitization, file ownership tracking via `file_uploads` table. `/uploads/:filename` and `/api/uploads/extract-text` both fail-closed when no ownership record exists. `X-Content-Type-Options: nosniff` on all file responses.
...
- **Flat credit pricing**: Users are charged a firm, fixed credit price (no estimates/approximations). The price includes a 1.3x safety margin baked in via `FLAT_PRICE_MULTIPLIER` and is rounded to friendly numbers via `niceRound()`. Post-debate settlement always settles at the reserved amount — no refunds or additional charges. Actual API costs are still logged for internal margin monitoring. Credits are only refunded on debate failure/timeout/cancellation.
- **Per-call topology pricing**: Credit cost is computed via `estimateDebateCostWithBufferInfo()` which models each of the 7 pipeline calls individually and splits costs into standard vs reasoning categories. Formula: `max(2, ceil(totalBuffered / 0.058))` where totalBuffered applies 1.20x buffer to standard model costs and 1.35x buffer to reasoning model costs. Chairman continuation calls (probability-weighted at 0.4) are included in the estimate. UTIL factor is 0.85. Stage output caps match actual maxTokens: S1=2500, S2=2048, S3=4000 (8000 for deliverables). Min 2 credits per debate.
